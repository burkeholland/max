...
import { writeWorkIqConfig } from "./copilot/mcp-config.js";
...
`);
  ...
  console.log(`  • Query your Microsoft 365 data via WorkIQ (email, calendar, Teams, docs)`);
  console.log();
  ...
  // ── Microsoft 365 / WorkIQ Setup ─────────────────────────
  console.log(`${BOLD}━━━ Microsoft 365 / WorkIQ (optional) ━━━${RESET}\n`);
  console.log(`WorkIQ connects Max to your Microsoft 365 data — email, calendar,`);
  console.log(`Teams messages, SharePoint documents, and people lookups.`);
  console.log();
  console.log(`${YELLOW}  Requirements:${RESET}`);
  console.log(`  • Microsoft 365 Copilot license`);
  console.log(`  • Tenant admin must grant consent (one-time, org-wide)`);
  console.log(`    ${DIM}See: https://github.com/microsoft/work-iq/blob/main/ADMIN-INSTRUCTIONS.md${RESET}`);
  console.log();

  const setupWorkIq = await askYesNo(rl, "Would you like to set up WorkIQ?");

  if (setupWorkIq) {
    console.log(`\n${BOLD}Tenant ID (optional)${RESET}\n`);
    console.log(`  Most users can leave this blank. Enterprise users may need their`);
    console.log(`  Entra tenant ID if the default ${DIM}"common"${RESET} endpoint doesn't work.`);
    console.log(`  ${DIM}Find yours at: https://portal.azure.com → Azure Active Directory → Overview${RESET}`);
    console.log();

    const tenantIdInput = (await ask(rl, `  Tenant ID ${DIM}(Enter to use "common")${RESET}: `)).trim();
    const tenantId = tenantIdInput || undefined;

    try {
      writeWorkIqConfig(tenantId);
      console.log(`\n${GREEN}  ✓ WorkIQ configured${tenantId ? ` (tenant: ${tenantId})` : ""}.${RESET}`);
      console.log(`  ${DIM}First use will open a browser for Microsoft authentication.${RESET}\n`);
    } catch (err) {
      console.log(`\n${YELLOW}  ⚠ Could not write WorkIQ config: ${err instanceof Error ? err.message : err}${RESET}`);
      console.log(`  ${DIM}You can set it up manually later.${RESET}\n`);
    }
  } else {
    console.log(`\n${DIM}  Skipping WorkIQ. You can always set it up later with: max setup${RESET}\n`);
  }
